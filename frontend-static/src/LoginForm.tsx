...
export function LoginForm({ isSubmitting, errorMessage, onSubmit }: LoginFormProps) {
  ...
  return (
    <form className="grid gap-3.5" onSubmit={handleSubmit} noValidate>
      <div className="grid gap-1.5 motion-safe:animate-auth-fade-up" style={{ animationDelay: "180ms" }}>
        <div className="flex items-baseline justify-between gap-4">
          <label className="text-[0.84rem] font-bold text-forest-900" htmlFor={identityId}>
            Identitas Akun
          </label>
          <span className="text-[0.72rem] text-forest-900/45">NIS, username, atau email</span>
        </div>
        <div
      ...
      <div className="grid gap-1.5 motion-safe:animate-auth-fade-up" style={{ animationDelay: "260ms" }}>
        <div className="flex items-baseline justify-between gap-4">
      ...
      >
        ...
        {isSubmitting ? (
          <span className="flex items-center gap-2">
            <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="2.5" className="h-4 w-4 motion-safe:animate-auth-spin">
              <path d="M10 3.5a6.5 6.5 0 1 0 6.5 6.5" strokeLinecap="round" />
            </svg>
            Memproses...
          </span>
        ) : (
      ...
      <p className="text-center text-[0.72rem] leading-normal text-forest-900/40 motion-safe:animate-auth-fade-up max-[700px]:hidden" style={{ animationDelay: "420ms" }}>
        Halaman login ini berjalan tanpa backend dan aman untuk kebutuhan demo.
      </p>
    </form>
  );
}
