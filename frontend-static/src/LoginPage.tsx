...
export function LoginPage() {
  ...
  async function handleLogin(values: { identity: string; password: string }) {
    ...
    setErrorMessage(`Mode demo aktif. Akun "${values.identity}" berhasil disimulasikan tanpa backend.`);
  }
      ...
      >
          ...
          <div className="motion-safe:animate-auth-fade-up">
            <div className="mb-5 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[0.78rem] font-bold leading-none text-mist-50/90">KH2-ManagementSystem</p>
              </div>
            </div>
          ...
          <div className="grid gap-2.5 lg:grid-cols-3 max-[900px]:grid-cols-1 max-[900px]:gap-2">
            {loginHighlights.map((item, index) => (
              ...
              >
                ...
                <h2 className="mb-1 text-[0.8rem] font-bold text-mist-50/90">{item.title}</h2>
                <p className="text-[0.72rem] leading-normal text-mist-100/60">{item.detail}</p>
              </article>
            ))}
          </div>
        </section>
        ...
        <section className="relative z-10 flex h-full min-w-0 items-center p-4 max-sm:p-3">
          ...
          >
            ...
            <LoginForm isSubmitting={isSubmitting} errorMessage={errorMessage} onSubmit={handleLogin} />
          </div>
        </section>
      </div>
    </main>
